module Gren.Syntax.Node exposing
    ( Node(..)
...
The purpose of this type is to add the information of the [`Range`](./Gren-Syntax-Range), i.e. where in the source code the
...
-}
type Node a
    = Node Gren.Syntax.Range.Range a
...
empty : a -> Node a
empty a =
    Node Gren.Syntax.Range.empty a
...
-}
range : Node a -> Gren.Syntax.Range.Range
range (Node r _) =
    r

...
With `gren/parser`, the above will work perfectly fine since
...
Currently reuses the `gren/parser` `DeadEnd` type to report problems
...
`'🙈🙉🙊'.length === 6`. Try it out! And since Gren runs in JavaScript, the offset
...
isSubCharWithoutLinebreak predicate offset string =
    -- https://github.com/gren/parser/blob/1.1.0/src/Gren/Kernel/Parser.js#L37
    let
        actualChar : String
        actualChar =
            String.slice offset (offset + 1) string
